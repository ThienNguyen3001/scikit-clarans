...
class TestAccelerationFeatures(unittest.TestCase):
    ...
    def test_deterministic_reproducibility(self):
        ...
        for ModelClass in (CLARANS, FastCLARANS):
            m1 = ModelClass(n_clusters=3, num_local=4, max_neighbors=50, random_state=42).fit(self.X)
            m2 = ModelClass(n_clusters=3, num_local=4, max_neighbors=50, random_state=42).fit(self.X)
    ...
    def test_multi_metric_support(self):
        ...
        for metric in ("cosine", "manhattan", "chebyshev", "euclidean"):
            m = FastCLARANS(n_clusters=3, num_local=2, max_neighbors=50, metric=metric, random_state=42).fit(self.X)
    ...
    def test_precomputed_metric_support(self):
        ...
        for ModelClass in (CLARANS, FastCLARANS):
            model = ModelClass(n_clusters=3, num_local=2, max_neighbors=50, metric="precomputed", random_state=42)
            model.fit(D)
            self.assertEqual(len(model.medoid_indices_), 3)
...
def run_benchmark():
    ...
    for n_samples in [2000, 5000]:
        ...
        print(f"\n--- Dataset: N = {n_samples:,}, Features = {n_features}, Clusters = {n_clusters} ---")
        ...
        for ModelClass in [CLARANS, FastCLARANS]:
            ...
            print(f"{ModelClass.__name__:<15} | Time: {elapsed*1000:6.1f} ms | Peak RAM: {peak/(1024**2):.2f} MB | Inertia: {m.inertia_:.2f}")
