...
.. code-block:: python
   ...
   def main():
       ...
       for N in Ns:
           ...
           CLARANS(n_clusters=4, num_local=1, random_state=42).fit(X)
           clarans_times.append(time.perf_counter() - t0)

           t0 = time.perf_counter()
           FastCLARANS(n_clusters=4, num_local=1, random_state=42).fit(X)
       ...
       ax.set_title("Runtime scaling (num_local=1)")
       ax.legend()
...
.. figure:: /_static/parameter_sensitivity.png
   ...
   Sensitivity of final cost / runtime to ``num_local`` and ``max_neighbors``.
