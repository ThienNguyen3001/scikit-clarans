...
def run_benchmark():
    ...
    print(f"Estimated full distance matrix size: ~{N_SAMPLES**2 * 8 / (1024**2):.1f} MB")

    X, _ = make_blobs(n_samples=N_SAMPLES, centers=N_CLUSTERS, n_features=N_FEATURES, random_state=RANDOM_STATE)

    clarans = CLARANS(n_clusters=N_CLUSTERS, numlocal=2, random_state=RANDOM_STATE)
    time_orig, mem_orig, medoids_orig = run_algorithm_profile("CLARANS (Original)", clarans, X)
    cost_orig = calculate_total_cost(X, medoids_orig)

    fast_clarans = FastCLARANS(n_clusters=N_CLUSTERS, numlocal=2, random_state=RANDOM_STATE)
    time_fast, mem_fast, medoids_fast = run_algorithm_profile("FastCLARANS", fast_clarans, X)
    ...
    print(f"  -> FastCLARANS uses {mem_diff:.1f}x more memory (due to full distance caching)")
    ...
    if cost_fast <= cost_orig:
        ...
        print("  -> Conclusion: CLARANS found a slightly better solution (due to randomness).")
